...
For local runs (webpack dev server + catalog API on port 8001), use **`oc login`** (and the usual cluster context) so the API can use your OpenShift session. **Prefer this for local testing** rather than the browser OAuth sign-in flow.
