...
For local runs (webpack dev server + catalog API on port 8001), use **`oc login`** against the **babydev** OpenShift cluster so the API uses that kube context. **Prefer this for local testing** rather than the browser OAuth sign-in flow.
