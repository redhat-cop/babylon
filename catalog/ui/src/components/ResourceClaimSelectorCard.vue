<template>
  <div class="co-catalog-page__grid-item">
    <a class="pf-c-card pf-m-hoverable catalog-tile-pf co-catalog-tile"
      @click="$router.push('/v/services/requests/' + resourceClaim.metadata.namespace + '/' + resourceClaim.metadata.name)"
    >
...
export default {
  name: 'ResourceClaimSelectorCard',
  components: {
  },
  props: ['resourceClaim'],
  computed: {
    ...
    iconName() {
      if (this.resourceClaim.metadata.annotations) {
        return this.resourceClaim.metadata.annotations['babylon.gpte.redhat.com/icon'];
      } else {
        return null;
      }
    },
    title() {
      if (this.resourceClaim.metadata.annotations) {
        return this.resourceClaim.metadata.annotations['babylon.gpte.redhat.com/displayName'] || this.resourceClaim.metadata.name
      }
      return this.resourceClaim.metadata.name
    },
    subtitle() {
      if (this.resourceClaim.metadata.annotations && this.resourceClaim.metadata.annotations['babylon.gpte.redhat.com/provider']) {
        return 'provided by ' + this.resourceClaim.metadata.annotations['babylon.gpte.redhat.com/provider'];
      }
    ...
    typeDisplay() {
      ...
      if (this.resourceClaim.metadata.labels) {
        ret = this.resourceClaim.metadata.labels['babylon.gpte.redhat.com/type'];
      }
      if (ret) {
        return ret;
      } else {
