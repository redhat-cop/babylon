...
const AnsibleRunLog: React.FunctionComponent<AnsibleRunLogProps> = ({
  ansibleRun,
}) => {
  return (
              ...
              { Object.entries(task.hosts || {}).map(([hostname, host]) =>
                <div key={hostname} className="ansible-run-log-task-host">
                  { (host.items || []).length > 0 ? (
                    ...
                    <div className="ansible-run-log-task-host-changed">
                      changed: [{hostname}]
                    </div>
                  ) : host.ok ? (
                    <div className="ansible-run-log-task-host-ok">
                      ok: [{hostname}]
                    </div>
                  ) : host.skipped ? (
                    <div className="ansible-run-log-task-host-skipped">
                      skipping: [{hostname}]
                    </div>
                  ) : host.failed ? (
          ...
          <div className="ansible-run-log-play-recap">
            ...
            { Object.entries(play.stats || {}).map(([hostname, host]) =>
              <div key={hostname} className="ansible-run-log-play-recap-host">
                <span className={`ansible-run-log-${host.failures > 0 ? 'failed' : host.unreachable > 0 ? 'failed' : host.changed > 0 ? 'changed' : 'default'}`}>{hostname}</span> :
                <span className={`ansible-run-log-${host.ok > 0 ? 'ok' : 'default'}`}>{' '}ok={host.ok}</span>
                <span className={`ansible-run-log-${host.changed > 0 ? 'changed' : 'default'}`}>{' '}changed={host.changed}</span>
                <span className={`ansible-run-log-${host.unreachable > 0 ? 'failed' : 'default'}`}>{' '}unreachable={host.unreachable}</span>
                <span className={`ansible-run-log-${host.failures > 0 ? 'failed' : 'default'}`}>{' '}failed={host.failures}</span>
                <span className={`ansible-run-log-${host.skipped > 0 ? 'skipped' : 'default'}`}>{' '}skipped={host.skipped}</span>
