...
function keywordMatch(workshop:Workshop, keyword:string): boolean {
  ...
  if (workshop.metadata.name.includes(keywordLowerCased)
...
const WorkshopsList: React.FunctionComponent<WorkshopsListProps> = ({
  ...
  useEffect(() => {
    if (workshopsFetchState?.canContinue && (
      workshopsFetchState.refreshing ||
      workshopsFetchState.filteredItems.length < workshopsFetchState.limit
    )) {
      fetchWorkshops();
    }
    return () => {
  ...
  useEffect(() => {
    if (!workshopsFetchState ||
      JSON.stringify(fetchNamespaces) !== JSON.stringify(workshopsFetchState?.namespaces)
    ) {
      reduceWorkshopsFetchState({
        type: 'startFetch',
  ...
  if (userIsAdmin === null ||
    (userIsAdmin && !userNamespacesFetchState?.finished)
  ) {
    return (
      <PageSection>
  ...
  if (serviceNamespaces.length === 1 && !serviceNamespaceName) {
    return (
      <Redirect to={`/workshops/${serviceNamespaces[0].name}`}/>
    );
  }
        ...
        <SelectableTable
          columns={
            (serviceNamespaceName ? [] : ['Project']).concat(
              ['Name', 'Registration', 'Users', 'Created At', 'Actions']
            )
          }
          ...
          rows={workshops.map((workshop:Workshop) => {
            ...
            const totalUserAssignments:number = workshop.spec.userAssignments ?
              workshop.spec.userAssignments.length : null;
            const claimedUserAssignments:number = workshop.spec.userAssignments ?
              workshop.spec.userAssignments.filter(item => item.assignment).length : null;
            const ownerReference = workshop.metadata?.ownerReferences?.[0];
            const owningResourceClaimName = (ownerReference && ownerReference.kind === 'ResourceClaim') ? ownerReference.name : null;
            ...
            cells.push(
              ...
              <>
                { totalUserAssignments ? (
                  `${claimedUserAssignments}/${totalUserAssignments}`
                ) : '-' }
              </>,
              // Created At
              <>
