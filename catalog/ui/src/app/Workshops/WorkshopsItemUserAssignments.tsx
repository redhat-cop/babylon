...
const WorkshopsItemUserAssignments: React.FunctionComponent<WorkshopsItemUserAssignmentsProps> = ({
  ...
  const haveUserNames = (workshop.spec.userAssignments || []).find(
    (ua) => ua.userName
  ) ? true : false;
  const resourceClaimNames = [...new Set(
    (workshop.spec.userAssignments || []).map((ua) => ua.resourceClaimName)
  )];
  ...
  async function bulkAssignUsers(emails:string[]) {
    const {workshop: updatedWorkshop, unassignedEmails, userAssignments} = await bulkAssignWorkshopUsers({
      emails: emails,
      workshop: workshop,
    })
    setBulkUserAssignmentMessage(
  ...
  async function updateUserAssignment({
    email, resourceClaimName, userName,
  } : {
    email: string;
  ...
  return (<>
    <BulkUserAssignmentModal key="bulk-user-assignment-modal"
          ...
          return (
            <Tr key={userAssignmentIdx}>
              { resourceClaimNames.length > 1 ? (
                <Td>
                  <Link key="services" to={`/services/${workshop.metadata.namespace}/${userAssignment.resourceClaimName}`}>
              ...
              <Td>
                <EditableText
                  ...
                  onChange={(email) => updateUserAssignment({
                    email: email,
                ...
                <DescriptionList isHorizontal>
                  ...
                  {userAssignment.messages ? (
                    <DescriptionListGroup>
                      ...
                      <DescriptionListDescription>
                        <div
                          dangerouslySetInnerHTML={{ __html: renderContent(userAssignment.messages.replace(/\n/g, "  +\n")) }}
                        />
                      </DescriptionListDescription>
                    </DescriptionListGroup>
                  ) : null }
    ...
    { bulkUserAssignmentMessage ? (
      <Text key="users-message">{bulkUserAssignmentMessage}</Text>
    ) : null }
  </>);
}
