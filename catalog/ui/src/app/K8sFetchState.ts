...
function reducePostItems(state:K8sFetchState, action:K8sFetchStateAction): K8sFetchState {
  ...
  // namespaces.
  const namespace:string|null = _continue ? state.namespace : state.namespace ? (
  ...
  if (state.refreshing) {
    const lastPostedItem = postedItems.length > 0 ? postedItems[postedItems.length - 1] : state.activity.lastRefreshedItem;
    ...
    if (canContinue && (
      (state.activity.lastItemToRefresh && -1 === compareK8sObjects(lastPostedItem, state.activity.lastItemToRefresh)) ||
      (!state.limit || state.limit > newFetchState.filteredItems.length)
    )) {
  ...
  if (action.refresh && !newFetchState.refreshing && (
