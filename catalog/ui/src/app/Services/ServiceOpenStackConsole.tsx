...
const ServiceOpenStackConsole: React.FunctionComponent<ServiceOpenStackConsoleProps> = ({
  resourceClaim,
}) => {
  ...
  const serverState = (
    ...
    subject => subject.openStackServers || []
  ).flat().find(s => s.id == serverId);
  ...
  const areAnySubjectsNotDesiredStarted = (resourceClaim?.status?.resources || []).some(r => (
    r.state?.kind === "AnarchySubject" && r.state?.spec?.vars.desired_state !== "started"
  ));
  const areAnySubjectsNotStarted = (resourceClaim?.status?.resources || []).some(r => (
    r.state?.kind === "AnarchySubject" && r.state?.spec?.vars.current_state !== "started"
  ));
  if (areAnySubjectsNotDesiredStarted) {
    return (
      <React.Fragment>
        <p>
          <Button variant="plain" onClick={() => startAllResourcesInResourceClaim(resourceClaim)}>Start</Button>
    ...
    return (
      <React.Fragment>
        <p>Waiting for environment to start. <Spinner isSVG size="lg" /></p>
      </React.Fragment>
    );
  }
    ...
    <React.Fragment>
      { serverDropdownItems.length > 1 ? (
        <Dropdown
          ...
          toggle={
            <DropdownToggle
              onToggle={() => setServerSelectIsOpen((v) => !v)}
      ...
      { serverState?.status === "ACTIVE" ? (
        <Button variant="primary" onClick={() => stopServer()}>Stop</Button>
      ) : serverState?.status === "SHUTOFF" ? (
        <Button variant="primary" onClick={() => startServer()}>Start</Button>
      ) : (
        <Button variant="primary" isDisabled={true}>Start</Button>
      )}
      {' '}
      <Button variant="primary"
        isDisabled={serverState?.status !== "ACTIVE"}
      ...
      >Reconnect</Button>
      { serverConsoleUrl ? (
        <iframe src={serverConsoleUrl} style={{width: "100%", height: "800px"}}></iframe>
      ) : null }
    </React.Fragment>
  );
}
