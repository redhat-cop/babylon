...
describe('CatalogItemForm Component', () => {
  test("When renders should display 'CatalogItem' properties and parameters", async () => {
    ...
    const sfidLabel = getByLabelText('Salesforce ID (Opportunity ID, Campaign ID, CDH or Project ID)');
  ...
  test('Description should be visible when hovering', async () => {
    ...
    const sfidLabel = await waitFor(() =>
      getByLabelText('Salesforce ID (Opportunity ID, Campaign ID, CDH or Project ID)')
    );
    const sfidDescriptionText = 'Salesforce Opportunity ID, Campaign ID, CDH or Project ID.';
