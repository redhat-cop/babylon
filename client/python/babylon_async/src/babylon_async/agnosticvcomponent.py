...
class AgnosticVComponent(K8sObject):
    ...
    def get_catalog_item_definition(self,
        ...
        if self.stage in ('dev', 'test', 'prod', 'event'):
            definition['metadata']['labels']["babylon.gpte.redhat.com/stage"] = self.stage

        if self.catalog_parameters is not None:
        ...
        # Add tenant cluster pool labels
        for tenant_component in tenant_cluster_components.values():
            definition['metadata']['labels'][f"tenant-cluster-pool.babylon.gpte.redhat.com/{tenant_component.name}"] = tenant_component.asset_uuid
    ...
    def get_resource_provider_definition(self) -> Mapping|None:
        ...
        if not self.catalog_disable:
            definition['metadata']['labels']["babylon.gpte.redhat.com/catalogItemName"] = self.name
            definition['metadata']['labels']["babylon.gpte.redhat.com/catalogItemNamespace"] = self.catalog_item_namespace
        ...
        if self.catalog_parameters:
            ...
            for parameter in self.catalog_parameters:
                ...
                default = None
                parameter_open_api_schema = deepcopy(parameter.get('openAPIV3Schema', {}))
    ...
    def get_tenant_cluster_pool_definition(self,
        environment_level: str,
        tenant_catalog_items: List[CatalogItem],
        ...
        for catalog_item in tenant_catalog_items:
            definition['spec']['tenantPools'][catalog_item.name] = {
                "minAvailable": 0,
                "provider": {
    ...
    async def get_tenant_catalog_items(self) -> List[CatalogItem]:
        """Return CatalogItems which use this AgnosticVComponent as a tenant cluster."""
        ret = []
        if self.sandbox_host is None:
            return ret
        # Find tenant catalog items by label
        async for catalog_item in self.client.list_catalog_items(
            label_selector=f"tenant-cluster-pool.babylon.gpte.redhat.com/{self.name}"
        ):
            # Only use catalog item if not deleting or pending delete
            if not catalog_item.is_deleting and not catalog_item.has_deleted_ops_annotation:
                ret.append(catalog_item)
        return ret
