...
class TenantClusterPool(K8sObject):
    ...
    def tenant_pools(self) -> List[TenantClusterPoolMergedTenantPool]:
        ...
        spec_and_status = {}
        for name, spec_value in (self.spec.tenant_pools or {}).items():
            spec_and_status[name] = [spec_value, None]

        for name, status_value in (self.status.tenant_pools or {}).items():
            if name in spec_and_status:
                spec_and_status[name][1] = status_value
            else:
                spec_and_status[name] = [None, status_value]
...
class TenantClusterPoolStatusTenantPoolResourceHandle:
    ...
    def claimed(self) -> bool:
        ...
        return self._definition.get('claimed', False)
...
class TenantClusterPoolMergedTenantPool:
    ...
    def __init__(self,
        name:str,
        spec:TenantClusterPoolSpecTenantPool,
        status:TenantClusterPoolStatusTenantPool,
    ):
        self.name = name
        self.spec = spec
        self.status = status
